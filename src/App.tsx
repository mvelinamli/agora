...
import { supabase } from './lib/supabaseClient'; // Supabase'i çağırıyoruz
...
function App() {
  ...
  const [currentUser, setCurrentUser] = useState("Misafir"); // Kullanıcı adı
  const [loading, setLoading] = useState(false);

  // --- VERİLER (Supabase'den gelecek) ---
  // Sunucular şimdilik sabit (İstersen bunları da veritabanına taşıyabiliriz)
  ...
  // --- SUPABASE BAŞLANGIÇ (INITIALIZATION) ---
  useEffect(() => {
    ...
    const initData = async () => {
      ...
      if (error) console.error("Kanal çekme hatası:", error);
      else {
        setChannels(data || []);
    ...
    // 2. Kanal Değişikliklerini Dinle (Biri yeni kanal açarsa anında gör)
  ...
  const handleSendMessage = async () => {
    ...
    else setInputText(""); // Başarılıysa kutuyu temizle
  };
  ...
  const handleAddChannel = async (type: string) => {
    ...
    if (name) {
      ...
      if (error) alert("Kanal oluşturulamadı: " + error.message);
    }
  };
  ...
  const handleDeleteChannel = async (id: number) => {
    if (confirm("Bu kanalı silmek istediğine emin misin?")) {
      ...
      else if (activeChannelId === id) setActiveChannelId(null);
    }
  };
  ...
  // Filtrelemeler
  const visibleChannels = channels.filter(c => c.server_id === activeServerId || c.server_id === undefined || c.server_id === null || c.server_id === 1); // Basitlik için ID 1 veya boş olanları göster
  ...
  // Giriş Ekranında Kullanıcı Adı Alıyoruz
  if (!isLoggedIn) return <LoginScreen onLogin={() => {
    const name = prompt("Kullanıcı adın ne olsun?", "Misafir");
  ...
  return (
    <div className={`h-screen w-screen ${theme.bg} flex items-center justify-center p-4 overflow-hidden relative font-sans ${theme.text} transition-colors duration-300`}>
      ...
      <CreateServerModal isOpen={isCreateServerOpen} onClose={() => setCreateServerOpen(false)} onCreate={() => alert("Sunucu oluşturma veritabanı henüz bağlanmadı.")} />
      ...
      <div className="flex w-full h-full max-w-[1600px] gap-4 relative z-10">
            ...
            <GlassPanel isDarkMode={isDarkMode} className="h-auto p-3 flex flex-col gap-3 shrink-0">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 cursor-pointer p-1.5 rounded-lg transition" onClick={() => setProfileOpen(true)}>
                  <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center font-bold text-white shadow-inner">{currentUser.charAt(0)}</div>
        ...
        <GlassPanel isDarkMode={isDarkMode} className="flex-1 flex flex-col relative overflow-hidden">
          {activeTab === 'feed' ? (
            <div className="flex-1 flex items-center justify-center text-2xl font-bold text-gray-500">Akış Sayfası (Geliştiriliyor)</div>
          ) : (
            <>
              ...
              <div className="flex flex-1 overflow-hidden">
                ...
                {showMembers && (
                  <div className={`w-60 border-l border-gray-500/10 ${isDarkMode ? 'bg-black/20' : 'bg-gray-50/50'} flex flex-col p-4`}>
                    <h3 className="text-xs font-bold text-gray-500 uppercase mb-4 tracking-wider">Çevrimiçi — 1</h3>
                    <div className="space-y-2">
                      <div className={`flex items-center gap-3 p-2 rounded-xl cursor-pointer hover:bg-gray-500/10`}>
                        <div className="relative">
                          <div className={`w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center text-xs text-white font-bold`}>{currentUser.charAt(0)}</div>
                          <div className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-gray-900 bg-green-500`}></div>
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className={`font-bold text-sm truncate ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>{currentUser}</div>
                          <div className="text-[10px] text-gray-500">Çevrimiçi</div>
                        </div>
                      </div>
                    </div>
                  </div>
